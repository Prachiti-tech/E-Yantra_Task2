...
class Edrone():
    ...
    ----------
        ...
        22) calcDistance :
            Helper function of schedular for calculating the distances between the coordinates
        23) schedular :
            Function sorts the coordinates and provides a sequenced target list. publishes a csv file of the coordinates
    """
    ...
    def pid(self):
        ...
        # Logging distance between waypoints
        rospy.logdebug("Wapoint distance: {}".format(self.waypoint_dist))
        ...
        # Limiting error values
        rospy.logdebug("long error: {}".format(self.error[0]))
        if abs(self.error[0]) >= self.max_error:
            self.error[0] = (self.assigned_error) * \
                math.copysign(1.0, self.error[0])

        rospy.logdebug("lat error: {}".format(self.error[1]))
        if abs(self.error[1]) >= self.max_error:
            self.error[1] = (self.assigned_error) * \
                math.copysign(1.0, self.error[1])

        if self.error[2]>=self.alt_max_err:
            self.error[2] = (self.assigned_error_alt) * \
                math.copysign(1.0, self.error[2])
    ...
    def controller(self):
        ...
        rospy.logdebug("Targets_achieved,n,states,x: {},{},{},{}".format(
            self.targets_achieved, self.n, self.states, self.x))
        rospy.logdebug(self.targets)
        self.alt_diff.publish(self.location.altitude -
                              self.buiding_locations[self.marker_id][2])
    ...
    def handle_marker(self):
        rospy.logdebug(
            "In Handle marker, marker id: {}".format(self.marker_id))
        ...
        if abs(self.err_x_m) < 0.2 and abs(self.err_y_m) < 0.2 and (self.location.altitude > self.targets[self.targets_achieved][2]-0.05 and self.location.altitude < self.targets[self.targets_achieved][2]+0.05):
            ...
            rospy.logdebug("x err , y err : {},{}".format(
                self.err_x_m, self.err_y_m))
            rospy.logdebug("self.targets[self.targets_achieved] : {}".format(
                self.targets[self.targets_achieved]))
            self.targets_achieved = 2

        else:
            rospy.loginfo("Navigating Drone to marker position")
            if self.count <= self.detection_count:
    ...
    def set_new_building_as_target(self):
        ...
        if self.targets[2][2] > self.buiding_locations[self.marker_id][2]:
            ...
            self.targets[0][2] = self.buiding_locations[self.marker_id][2] + 12
            self.targets[1][2] = self.buiding_locations[self.marker_id][2] + 12
    ...
    def set_location_using_err(self):
        ...
        self.targets[-1][2] = self.buiding_locations[self.marker_id][2]+0.2
        self.target_list()
        self.targets_achieved = 1
    ...
    def landing_control(self):
        ...
        # checking if the box is in the 2nd state
        if self.states == 2:
            ...
            if self.bottom_count == 0:
                ...
                #Condition for the code executed while delivering a parcel
                elif self.buiding_locations[self.marker_id][3] == "DELIVERY":
                    self.activate_gripper(shall_i=True)
                    #checking if the box has been gripped
                ...
                #Condition for the code executed while picking up a return parcel
                elif self.buiding_locations[self.marker_id][3] == "RETURN ":
                    self.activate_gripper(shall_i=True)
                    if self.gripper_data and self.states == 2:
                        self.targets_achieved = 0
                        self.marker_id += 1
        ...
        #Checking the state for Dropping a return parcel
        elif self.buiding_locations[self.marker_id][3] == "RETURN " and self.states == 5:
            self.drone_cmd.rcRoll = self.base_pwm - self.ouput[0]
            self.drone_cmd.rcPitch = self.base_pwm - self.ouput[1]
            self.drone_cmd.rcYaw = self.base_pwm
            rospy.logdebug("Dropping return, Alt error: {} : Gripper status: {}".format(self.error[2],self.gripper_data))

            #Checking if the drone landed on the building for releasing the parcel
            if abs(self.location.altitude-16.757981) < 1:
                self.activate_gripper(shall_i=False)
                self.box_grabbed = 0
                if self.gripper_data:
                    self.bottom_count = 0
        ...
        # Landing the drone
            ...
            self.drone_cmd.rcPitch = self.base_pwm
            self.drone_cmd.rcYaw = self.base_pwm
        if self.buiding_locations[self.marker_id][3] == "DELIVERY" and self.states == 5 and self.gripper_data:
            self.drone_cmd.rcRoll = self.base_pwm - self.ouput[0]
            self.drone_cmd.rcPitch = self.base_pwm - self.ouput[1]

        # Changing the allowed error to its maximum once the box is picked up to fast up the drone
        if self.gripper_data and not self.buiding_locations[self.marker_id][3] == "RETURN ":
            self.allowed_lat_error = 0.000004517/4
            self.allowed_lon_error = 0.0000047487/4
    ...
    def get_gps_coordinates(self):
        ...
        #Dictionary for the coordinates of  rows and colomn of the delivery and return grids
        rows = {'1': 72.000142461, '2': 72.000142461 +
                (0.000014245), '3': 72.000142461 + (2*0.000014245)}
        columns = {'A': 18.9998102845, 'B': 18.9998102845 +
                   (0.000013552), 'C': 18.9998102845 + (2*0.000013552)}
        return_rows = {'1': 72.000142461, '2': 72.000142461 +
                       (0.000014245), '3': 72.000142461 + (2*0.000014245)}
        return_columns = {'X': 18.9999367615, 'Y': 18.9999367615 +
                          (0.000013552), 'Z': 18.9999367615 + (2*0.000013552)}
        ...
        with open(path, 'r') as file:
            ...
            for row in csvreader:
                lines_count += 1
                rospy.logdebug("CSV: {} {} {}".format(
                    csvreader.line_num, row, lines_count))
        ...
        rospy.logdebug("lenght List of contents: {}".format(
            len(list_of_contents)))
        #Loop for decoding the CSV file and storing the return and delivery coordinates
        for i in range(len(list_of_contents)):
            if list_of_contents[i][0] == "DELIVERY":
                cell = list_of_contents[i][1]
                box = [rows[cell[1]], columns[cell[0]], altitude] 
                ...
                coordinate = [float(item)
                              for item in list_of_contents[i][2].split(";")]
                z = coordinate[0]
                coordinate[0] = coordinate[1]
                coordinate[1] = z
                box_coord[tuple(coordinate[:3])] = box[:3]
                ...
                coordinate = [float(item)
                              for item in list_of_contents[i][1].split(";")]
                z = coordinate[0]
                coordinate[0] = coordinate[1]
                coordinate[1] = z
                coordinate.append(list_of_contents[i][0])
                ...
                ret_grid = [return_rows[cell[1]],
                              return_columns[cell[0]], altitude]
                box_coord[tuple(coordinate[:3])] = ret_grid[:3]
        rospy.logdebug("targets : \n{}".format(self.targets))
        rospy.logdebug("building locations : \n{}".format(
            self.buiding_locations))
        delivery = []
        returns = []
        #Loop for storing deliveries and returns into a different file
        ...
        self.schedular(delivery, returns, box_coord)

    
    def calcDistance(self, x2, y2, x1=72.0002184152, y1=18.9998887669):
        # calculates distance
        dist = math.sqrt(pow(110692.0702932625*(x2-x1), 2) +
                         pow(-105292.0089353767*(y2-y1), 2))

        return dist

    def schedular(self, delivery, returns, box_coord):
        delivery_dict = {}
        return_dict = {}
        new_targets = []
        max_val = 0
        ...
        #Loop for creating a list of distances from origin to the corresponding coordinates
        for i in range(len(delivery)):

            delivery_dict[delivery[i]] = self.calcDistance(
                delivery[i][0], delivery[i][1])
            return_dict[returns[i]] = self.calcDistance(
                returns[i][0], returns[i][1])

        #Sorting the file wrt to distance in descending order
        sorted_deliveries = OrderedDict(
            sorted(delivery_dict.items(), reverse=True, key=lambda x: x[1]))
        delivery = sorted_deliveries.keys()
        #Sorted distance list adn their keys
        ...
        #Loop for creating a combined sequenced list of deliveries and returns
        for i in delivery:
            for j in returns:
                current_delivery_to_return_distance = self.calcDistance(
                    j[0], j[1], i[0], i[1])
            ...
            path_2 = delivery_to_return_distance + \
                return_dict[delivery_to_return_coordinates]
            # Checking if the path is feasable considering its distsnce 
            if path_2-delivery_to_return_distance+path_1 < self.path_cost_thresh:
                # Delivery coordinates
                rospy.logdebug("{}".format( path_2-delivery_to_return_distance+path_1))
                del_coord = list(i)
                box = list(box_coord[tuple(del_coord)])
                csv_del_cd.append(tuple(del_coord))
                box.append("DELIVERY")
                self.buiding_locations.append(box)
                del_coord.append("DELIVERY")
                self.buiding_locations.append(del_coord)
                # Return coordinates
                ret_coord = list(delivery_to_return_coordinates)
                box = list(box_coord[tuple(ret_coord)])
                csv_ret_cd.append(tuple(ret_coord))
                ret_coord.append("RETURN ")
                self.buiding_locations.append(ret_coord)
                box.append("RETURN ")
                self.buiding_locations.append(box)
        ...
        #Code for writing the sequenced list into a file named sequenced_manifest.csv 

        # del_dict = {'A1': (72.000142461, 18.9998102845, 16.757981), 'A2': (72.000156707, 18.9998102845, 16.757981), 'A3': (72.000170953, 18.9998102845, 16.757981), 'B1': (72.000142461, 18.999823836,  16.757981), 'B2': (
        #     72.000156707, 18.999823836, 16.757981), 'B3': (72.000170953,18.999823836,16.757981), 'C1': (72.000142461, 18.999837387, 16.757981), 'C2': (72.000156707, 18.999837387, 16.757981), 'C3': (72.000170953, 18.999837387, 16.757981)}
        # ret_dict = {'X1 ': (72.000142461, 18.9999367615, 16.757981), 'X2 ': (72.000156707, 18.9999367615, 16.757981), 'X3': (72.000170953, 18.9999367615, 16.757981), 'Y1 ': (72.000142461, 18.999950313, 16.757981), 'Y2 ': (
        #     72.000156707, 18.999950313, 16.757981), 'Y3 ': (72.000170953, 18.999950313, 16.757981), 'Z1 ': (72.000142461, 18.999963864, 16.757981), 'Z2': (72.000156707, 18.999963864, 16.757981), 'Z3 ': (72.000170953, 18.999963864, 16.757981)}
        # del_dict = {v: k for k, v in del_dict.iteritems()}
        # ret_dict = {v: k for k, v in ret_dict.iteritems()}
        # with open(os.path.join(rospkg.RosPack().get_path("vitarana_drone"), "scripts/sequenced_manifest.csv")) as seq_csv:
        #     csv_writer = csv.writer(seq_csv, delimiter=',', quotechar='')
        #     csv_writer.writerow(["Type", "Origin", "Destination"])
        #     for i in range(len(csv_del_cd)):
        #         csv_writer.writerow(
        #             ["Delivery", ret_dict[csv_del_cd[i][0]], csv_del_cd[i]])
        #         csv_writer.writerow(
        #             ["Return", csv_ret_cd[i], ret_dict[csv_ret_cd[i][0]]])

        self.targets[2] = [self.location.longitude,
                           self.location.latitude, self.location.altitude]
        self.set_new_building_as_target()
    # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
